...
import {
  getImproveAgilityFocusMeta,
  getImproveCategory,
  getImproveSkill,
  listDrillsForAgilityFocus,
  skillHasAgilityFocus,
} from '@/lib/improveContent';

export default function ImproveAgilityFocusDrillListScreen() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const { categoryId, skillId, agilityFocusId } = useLocalSearchParams<{
    categoryId: string;
    skillId: string;
  ...
  const focusArea = agilityFocusId ? getImproveAgilityFocusMeta(agilityFocusId) : undefined;
  const drills =
    categoryId && skillId && agilityFocusId
      ? listDrillsForAgilityFocus(categoryId, skillId, agilityFocusId)
      : [];

  if (!category || !skill || !focusArea || !skillHasAgilityFocus(skill)) {
    return (
